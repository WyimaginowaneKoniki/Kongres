...
function WorkView() {
  ...
  const [open, setOpen] = useState([
    true,
    ...new Array(versions.length - 1).fill(false),
  ]);

  function handleOnClick(item) {
    if (open[item]) open[item] = false;
    else open[item] = true;
    ...
    if (user === "reviewer") {
      reviewsList.push(
        <CurrentVersion
          ...
          alternativeTextParticipant={
            version.reviewer.alternativeTextParticipant
          }
  ...
  return (
    <div className={style.main}>
      {user === "participant" ? (
        ...
        <p className={style.path}>
          Scientific works / <span className={style.title}>{work.title}</span>
        </p>
      )}
